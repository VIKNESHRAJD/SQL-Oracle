...
WHERE PRICE IN
      (SELECT PRICE
       FROM PRINTER P
       JOIN PRODUCT_INFO PI ON P.MODEL_NO = PI.MODEL_NO
       WHERE PI.MAKER = 'IBM');
...
SELECT RAM, COUNT(*) AS NUMBER_OF_PCS
FROM PC
GROUP BY RAM;

-- 7. For each value of HD, list the number of PCs.
SELECT HD, COUNT(*) AS NUMBER_OF_PCS
FROM PC
GROUP BY HD;
...
WHERE AGE = (SELECT MAX(AGE) FROM WORKER)
   OR AGE = (SELECT MIN(AGE) FROM WORKER);
...
-- 4. Solve the above query using a single INSERT statement.
INSERT INTO LAPTOP
       (MODEL_NO, SPEED, RAM, HD, SCREENSIZE, PRICE)
SELECT REPLACE(MODEL_NO, 'PC', 'LP'),
       SPEED,
